    ...
    this.$get = function($state, layoutSizes) {
      ...
      function _factory() {
        ...
        this.getAuthorizedMenuItems = function(user) {
            var states = defineMenuItemStates();
            var UsersPremmisions = user.role;

            var menuItems = states.filter(function(item) {
              var ItemPremmisions = item.authRoles;
              var matches = UsersPremmisions.filter(function(item){
                if(ItemPremmisions) {
                  return ItemPremmisions.indexOf(item) > -1
                }
              });

                return item.level == 0 && matches.length > 0;
            ...
            menuItems.forEach(function(item) {
                ...
                var children = states.filter(function(child) {
                    var childPremmisions = child.authRoles;

                    var matches = UsersPremmisions.filter(function(item){
                      if(childPremmisions) {
                        return childPremmisions.indexOf(item) > -1
                      }
                    });

                    // console.log('childPremmisions:');
                    // console.log(childPremmisions);
                    // console.log(item.name);

                    return child.level == 1 && child.name.indexOf(item.name) === 0 && matches.length > 0;
                });
                item.subMenu = children.length ? children : null;
            });
